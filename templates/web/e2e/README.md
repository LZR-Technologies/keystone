...
| 3. **Integration**   | Data access against a **real database** — mocks lie: they encode what you believe the database does, and pass while production fails. Runs the real migration files against `@electric-sql/pglite`, an in-process WASM Postgres — no daemon, no `DATABASE_URL`, so this layer runs by default (not skipped) on a laptop or any CI runner | Vitest     | `src/__tests__/integration/tenant-isolation.test.ts`  |
